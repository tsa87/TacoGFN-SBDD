...
class PrecalculationModule():
    def __init__(
        ...
        model = AffinityModule(backbone, head, config.THRESHOLD.FOCUS, config.THRESHOLD.BOX, head_checkpoint['absolute_score_threshold'])
    ...
    def __parse_protein(
        ...
        pocket_positions, pocket_features = pointcloud.get_protein_pointcloud(pocket_obj)
        token_positions, token_classes = token_inference.get_token_informations(pocket_obj)
        ...
        pocket_image = np.asarray(
            self.in_voxelizer.forward_features(pocket_positions, center, pocket_features, radii=self.protein_radii),
            np.float32
        )
    ...
    def __ready_to_calculate(
        ...
        with torch.amp.autocast(self.device, enabled=self.config.AMP_ENABLE):
            ...
            indices = self.model._get_valid_tokens(tokens, token_scores, cavity_narrow, cavity_wide)   # [Ntoken']
