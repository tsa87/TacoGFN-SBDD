"""
Fragment building environment from:
...
class FragMolBuildingEnvContext(GraphBuildingEnvContext):
    ...
    def __init__(
        ...
        if fragments is None:
            smi, stems = zip(*fragment_const.GFLOWNET_FRAGMENTS)
        else:
    ...
    def aidx_to_GraphAction(
        self, g: gd.Data, action_idx: Tuple[int, int, int], fwd: bool = True
    ):
    ...
    def graph_to_Data(self, g: Graph) -> gd.Data:
        ...
        for i, n in enumerate(g.nodes):
            ...
            # attributes.
            edge_has_no_attr = bool(
                len(g.edges[list(g.edges(i))[0]]) == 0
                if degrees[i] == 1
                else degrees[i] == 0
            )
        ...
        stop_mask = (
            zeros((1, 1))
            if has_unfilled_attach or not len(g)
            else np.ones((1, 1), np.float32)
        )
