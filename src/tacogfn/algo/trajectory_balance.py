...
from src.tacogfn.algo.graph_sampling import (
    GraphSampler,
    PharmacoCondGraphSampler,
    PocketCondGraphSampler,
)
...
class TrajectoryBalance(GFNAlgorithm):
    ...
    def __init__(
        ...
        if self.cfg.variant == TBVariant.SubTB1:
            ...
            self._init_subtb(
                torch.device("cuda")
            )  # TODO: where are we getting device info?
    ...
    def get_idempotent_actions(
        ...
        for i in nz:
            ...
            if aidx == iaction:
                continue
            ga = self.ctx.aidx_to_GraphAction(
    ...
    def construct_batch(self, trajs, cond_info, log_rewards):
        ...
        if self.model_is_autoregressive:
            ...
            actions = [
                self.ctx.GraphAction_to_aidx(g, i[1])
    ...
    def subtb_loss_fast(self, P_F, P_B, F, R, traj_lengths):
        ...
        for ep in range(traj_lengths.shape[0]):
            ...
            total_loss[ep] = (F_start - F_end + P_F_sums - P_B_sums).pow(2).sum() / car[
                T
            ]
        return total_loss
...
class PharmacophoreTrajectoryBalance(TrajectoryBalance):
    ...
    def create_training_data_from_own_samples(
        ...
        pharmaco_batch = gd.Batch.from_data_list(
            self.pharmaco_dataset.get_datalist_from_idxs(cond_info["pharmacophore"])
        ).to(dev)
    ...
    def compute_batch_losses(
        ...
        pharmacophore_list = self.pharmaco_dataset.get_datalist_from_idxs(
            batch.pharmacophore
        )
        ...
        batch_with_pharmacophore = merge_pocket_and_molecule_data_list(
...
class PocketTrajectoryBalance(TrajectoryBalance):
    def __init__(
        self,
        env: GraphBuildingEnv,
        ctx: GraphBuildingEnvContext,
        pocket_dataset: PocketDB,
        pharmaco_dataset: PharmacoDB,
        rng: np.random.RandomState,
        cfg: Config,
    ):
        ...
        self.pharmaco_dataset = pharmaco_dataset
        self.graph_sampler = PocketCondGraphSampler(
            ctx,
            env,
            pocket_dataset,
            pharmaco_dataset,
    ...
    def create_training_data_from_own_samples(
        ...
        keys = self.pharmaco_dataset.get_keys_from_idxs(
            cond_info["pharmacophore"].tolist()
        )
        pocket_list = self.pocket_dataset.get_from_keys(keys)
    ...
    def compute_batch_losses(
        ...
        """
        # Get pharmacophore batch
        keys = self.pharmaco_dataset.get_keys_from_idxs(batch.pharmacophore.tolist())
        pocket_list = self.pocket_dataset.get_from_keys(keys)
