        ...
        for t in range(self.max_len):
            ...
            if random_action_prob > 0:
                ...
                is_random_action = torch.tensor(
                    self.rng.uniform(size=len(torch_graphs)) < random_action_prob,
                    device=dev,
                ).float()
            ...
            graph_actions = [
                self.ctx.aidx_to_GraphAction(g, a)
    ...
    def sample_backward_from_graphs(
        ...
        if random_action_prob > 0:
            raise NotImplementedError(
                "Random action not implemented for backward sampling"
            )
        ...
        while sum(done) < n:
            ...
            if model is not None:
                ...
                bck_cat = GraphActionCategorical(
                    ...
                    keys=[
                        ...
                        GraphTransformerGFN._graph_part_to_key[
                            GraphTransformerGFN._action_type_to_graph_part[t]
                        ]
            ...
            graph_bck_actions = [
                self.ctx.aidx_to_GraphAction(g, a, fwd=False)
...
class PharmacoCondGraphSampler(GraphSampler):
    """A helper class to sample from GraphActionCategorical-producing models"""
    ...
    def sample_backward_from_graphs(
        ...
        while sum(done) < n:
            ...
            not_done_mask = torch.tensor(done, device=dev).logical_not()
            if model is not None:
                # Forward pass with pharmacophore information
                pharmacophore_graphs = [
                    pharmacophore_list[i] for i in not_done(range(n))
                ]
                batch = merge_pharmacophore_and_molecule_data_list(
                    pharmacophore_graphs, torch_graphs
                ).to(dev)
                _, bck_cat, *_ = model(batch, cond_info[not_done_mask])
                # _, bck_cat, *_ = model(
                #     self.ctx.collate(torch_graphs).to(dev), cond_info[not_done_mask]
                # )
            else:
